    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                # Track canonical model identity from the provider response
                # (P2/P3). Providers may serve a different canonical model than
                # the requested id. agent.model keeps the REQUESTED id
                # (cost/config contract); agent.response_model holds what the
                # provider actually served and feeds /status, pricing and
                # persistence.
                _resp_model = getattr(response, "model", None)
                if isinstance(_resp_model, str) and _resp_model.strip():
                    agent.response_model = _resp_model.strip()
                ...
                if hasattr(response, 'usage') and response.usage:
                    ...
                    _served = ""
                    if (
                        getattr(agent, "response_model", None)
                        and agent.response_model != agent.model
                    ):
                        _served = f" served={agent.response_model}"
                    logger.info(
                        "API call #%d: model=%s provider=%s in=%d out=%d total=%d latency=%.1fs%s%s",
                        agent.session_api_calls, agent.model, agent.provider or "unknown",
                        prompt_tokens, completion_tokens, total_tokens,
                        api_duration, _cache_pct, _served,
                    )

                    cost_result = estimate_usage_cost(
                        getattr(agent, "response_model", None) or agent.model,
                        canonical_usage,
                        provider=agent.provider,
                        base_url=agent.base_url,
                    ...
                    if agent._session_db and agent.session_id:
                        try:
                            ...
                            agent._session_db.update_token_counts(
                                ...
                                cache_write_tokens=canonical_usage.cache_write_tokens,
                                reasoning_tokens=canonical_usage.reasoning_tokens,
                                response_model=getattr(agent, "response_model", None),
