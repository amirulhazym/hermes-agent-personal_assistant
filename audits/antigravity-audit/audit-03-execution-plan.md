<!-- ===== OVERHAUL ADDENDUM — 2026-07-09 (Antigravity deep-audit extension) =====
...
> Auditor: Antigravity (executor). Jane/MJ (native VPS agent) = verifier only.
...
2. **PC → VPS:** Antigravity edits → `git commit` (hermes-live) → VPS `git pull`. **Needs user "yes" per AGENTS.md.**
...
| O6 | F-22, F-25 (med-auto-confirm) | CRIT | M | yes (med pipeline) |
...
### O6 [CRITICAL] — Fix `med-auto-confirm` hook false-positives (Pattern G) [F-22, F-25]
- **File:** `hooks/med-auto-confirm/handler.py`, `hooks/med-auto-confirm/HOOK.yaml`, `scripts/chain_monitor.sh`.
- **Change:**
  - O6a / G-1: Tighten `SLOT_RE` to require medication intent context or specific drug name patterns.
  - O6b / G-2: Add timestamp validation to reject future/impossible times (more than 2h ahead of MYT).
  - O6c / G-3: Add context guard to check for confirmation intent words (dah makan, done, yes, etc.).
  - O6d / G-4: Fix `_already_logged()` in handler.py to check for validity (not just existence) to prevent lockups.
  - O6e / G-5: Extract day-roll boundary reset block out of the should-fire gate in `chain_monitor.sh` so it runs every tick.
  - O6f / G-6: Implement audit logging to `~/.hermes/logs/med-auto-confirm-audit.log`.
  - O6g / G-7: Add a regression test suite under `tests/` for false-positives.
- **Verify:** Run hook test cases; assert that "A tu 20:00 tu" does NOT confirm and that genuine replies confirm.
- **Human gate:** YES — med pipeline.
- **Why it matters:** Highest immediate safety priority. Reconciles hook-level state mutations before building the new med engine (O8).
- **Dependency chain:** O6 → O8 (do not build the new med engine while the hook can silently corrupt its state).
