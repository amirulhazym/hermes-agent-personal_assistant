...
class SessionDB:
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        self._conn = None
        # Feature-detect the canonical_model column (F1). Pre-migration DBs
        # keep working: update_token_counts compiles SQL without the column.
        self._has_canonical_model = False
        try:
            if read_only:
            ...
            def _connect_and_init():
                ...
                try:
                    _cols = [
                        r[1] for r in self._conn.execute("PRAGMA table_info(sessions)")
                    ]
                    self._has_canonical_model = "canonical_model" in _cols
                except Exception:
                    self._has_canonical_model = False
    ...
    def update_session_model(self, session_id: str, model: str) -> None:
        ...
        The ``canonical_model`` column (model actually served by the
        provider) is reset to NULL — a model switch invalidates the previous
        canonical until the next API call records a fresh one (F5/F7).
        """
        def _do(conn):
            if self._has_canonical_model:
                conn.execute(
                    "UPDATE sessions SET model = ?, canonical_model = NULL WHERE id = ?",
                    (model, session_id),
                )
            else:
                conn.execute(
    ...
    def update_token_counts(
        ...
        ``response_model`` (the canonical model the provider actually served,
        which may differ from the requested ``model`` id) is persisted
        last-write-wins when non-empty and the column exists
        (feature-detected at init).
        """
        ...
        # F1: pre-migration DBs have no canonical_model column — compile the
        # UPDATE without it rather than crashing token persistence.
        canonical_col = ""
        canonical_params: tuple = ()
        if self._has_canonical_model and response_model:
            canonical_col = (
                "canonical_model = CASE WHEN ? IS NOT NULL AND ? != '' "
                "THEN ? ELSE canonical_model END,"
            )
            canonical_params = (response_model, response_model, response_model)
        if absolute:
            sql = f"""UPDATE sessions SET
                  ...
                  billing_provider = CASE WHEN ? IS NOT NULL THEN ? ELSE billing_provider END,
                  billing_base_url = CASE WHEN ? IS NOT NULL THEN ? ELSE billing_base_url END,
                  billing_mode = CASE WHEN ? IS NOT NULL THEN ? ELSE billing_mode END,
