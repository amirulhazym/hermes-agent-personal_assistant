...
Fixed: minority-fill detection + silent captcha refresh detection.
"""
...
async def solve_captcha(page):
    """Solve QRYPTY captcha by detecting MINORITY fill values."""
...
async def create_one(browser, username, nickname, max_attempts=4):
    ...
    try:
        ...
        for attempt in range(max_attempts):
            ...
            # Detect silent refresh: Answer field is now empty = captcha changed
            ...
            # Still on register with answer still filled — maybe username taken
...
async def main():
    print(f"{'='*55}")
    print(f"  QRYPTY Mail Batch v4 — {TARGET} accounts")
