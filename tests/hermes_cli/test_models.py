...
class TestModelIds:
    ...
    def test_openai_api_uses_live_ids_intersected_with_dynamic_agentic_catalog(self, monkeypatch):
        """Direct OpenAI discovery must stay live without exposing non-agent API products."""
        from hermes_cli.models import provider_model_ids

        monkeypatch.setenv("OPENAI_API_KEY", "test-key")
        monkeypatch.delenv("OPENAI_BASE_URL", raising=False)
        live_ids = ["gpt-4o", "text-embedding-3-small", "gpt-5.6-luna", "gpt-image-2", "gpt-realtime-2.1"]
        dynamic_agentic_ids = ["gpt-5.6-luna", "gpt-4o", "gpt-realtime-2.1"]

        with (
            patch("hermes_cli.models.fetch_api_models", return_value=live_ids),
            patch("agent.models_dev.list_agentic_models", return_value=dynamic_agentic_ids),
        ):
            result = provider_model_ids("openai-api", force_refresh=True)

        assert result == ["gpt-4o", "gpt-5.6-luna"]
...
class TestDeprecatedAliasFiltering:
    """P1: deprecated provider aliases must not surface in the picker when the
    live catalog omits them (proven 2026-08-07: DeepSeek live /v1/models lists
    only deepseek-v4-flash + deepseek-v4-pro; deepseek-chat/deepseek-reasoner
    are legacy aliases). Aliases still served by the live catalog are kept."""
    ...
    class _FakeProfile:
        ...
        def fetch_models(self, api_key="", base_url=None):
            return list(self.live)

    @staticmethod
    def _install_fake_deepseek(monkeypatch, live_models):
        from providers import get_provider_profile as _real_get_profile

        fake = TestDeprecatedAliasFiltering._FakeProfile()
        fake.live = list(live_models)

        def _fake_get_profile(p):
            if p == "deepseek":
                return fake
            return _real_get_profile(p)

        monkeypatch.setattr("providers.get_provider_profile", _fake_get_profile)
        monkeypatch.setattr(
            "hermes_cli.auth.resolve_api_key_provider_credentials",
            lambda p: {"api_key": "test-key", "base_url": "https://api.deepseek.com/v1"},
        )
        return fake

    def test_picker_excludes_legacy_aliases_when_live_omits_them(self, monkeypatch):
        from hermes_cli.models import provider_model_ids

        self._install_fake_deepseek(monkeypatch, ["deepseek-v4-flash", "deepseek-v4-pro"])
        ...
        assert set(result) == {"deepseek-v4-pro", "deepseek-v4-flash"}

    def test_picker_keeps_aliases_if_live_catalog_serves_them(self, monkeypatch):
        from hermes_cli.models import provider_model_ids

        self._install_fake_deepseek(
            monkeypatch, ["deepseek-v4-flash", "deepseek-v4-pro", "deepseek-chat"]
        )
        result = provider_model_ids("deepseek", force_refresh=True)
        assert "deepseek-chat" in result

    def test_static_provider_models_has_no_legacy_aliases(self):
        from hermes_cli.models import _PROVIDER_MODELS

        assert _PROVIDER_MODELS["deepseek"] == ["deepseek-v4-pro", "deepseek-v4-flash"]

    def test_cached_provider_model_ids_filters_stale_cache_entry(self, monkeypatch):
        import time

        from hermes_cli import models as m

        monkeypatch.setenv("DEEPSEEK_API_KEY", "test-key")
        fp = m._credential_fingerprint("deepseek")
        monkeypatch.setattr(
            m,
        ...
        result = m.cached_provider_model_ids("deepseek")
        assert "deepseek-chat" not in result
        assert "deepseek-reasoner" not in result
        assert set(result) == {"deepseek-v4-pro", "deepseek-v4-flash"}
...
class TestLegacyAliasFullPurge:
    """Full purge (2026-08-07): deepseek-chat / deepseek-reasoner must not be
    selectable, resolvable, or reroutable anywhere. Typed input fails loud —
    it must never leak to OpenRouter's stale catalog."""

    def test_typed_alias_does_not_resolve_to_any_provider(self):
        from hermes_cli.models import detect_provider_for_model

        for alias in ("deepseek-chat", "deepseek-reasoner"):
            # current_provider different from deepseek: must NOT reroute
            # (previously fell through to the OpenRouter catalog).
            assert detect_provider_for_model(alias, "openai-codex") is None

    def test_typed_alias_does_not_resolve_on_deepseek_itself(self):
        from hermes_cli.models import detect_provider_for_model

        assert detect_provider_for_model("deepseek-chat", "deepseek") is None

    def test_no_alias_resolution_via_builtin_direct_aliases(self):
        from hermes_cli.model_switch import resolve_alias, _ensure_direct_aliases

        _ensure_direct_aliases()
        assert resolve_alias("deepseek-chat", "opencode-zen") is None
        assert resolve_alias("deepseek-reasoner", "opencode-zen") is None

    def test_normalize_falls_back_to_v4_flash(self):
        from hermes_cli.model_normalize import _normalize_for_deepseek

        assert _normalize_for_deepseek("deepseek-chat") == "deepseek-v4-flash"
        assert _normalize_for_deepseek("deepseek-r1") == "deepseek-v4-flash"
