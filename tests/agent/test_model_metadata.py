...
class TestDeepSeekContextResolution:
    """P5 regression: DeepSeek V4 family + legacy aliases resolve to 1M.

    Verified 2026-08-07 against api-docs.deepseek.com pricing page
    (deepseek-v4-flash CONTEXT LENGTH = 1M) and a live alias probe
    (model=deepseek-v4-flash → response.model=deepseek-v4-flash). This guards
    the documented behavior so the 128K/84% misreading cannot regress.
    """

    @pytest.mark.parametrize(
        "model", ["deepseek-v4-flash", "deepseek-v4-flash", "deepseek-v4-flash", "deepseek-v4-pro"]
    )
    def test_v4_family_and_aliases_resolve_1m(self, model):
        from agent.model_metadata import get_model_context_length

        ctx = get_model_context_length(model=model, provider="deepseek")
        assert ctx == 1_000_000
