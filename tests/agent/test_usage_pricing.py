...
class TestDeepSeekV4FlashPricing:
    """F3: deepseek-v4-flash must have an official pricing entry — it is the
    canonical serving model for the legacy deepseek-v4-flash alias and the
    recommended picker model (verified against api-docs.deepseek.com
    2026-08-07: $0.14 cache-miss input, $0.0028 cache-hit, $0.28 output)."""

    def test_v4_flash_has_official_pricing_entry(self):
        ...
        assert entry.pricing_version == "deepseek-pricing-2026-08-07"

    def test_v4_flash_estimate_not_unknown(self):
        ...
        result = estimate_usage_cost(
            "deepseek-v4-flash", usage, provider="deepseek"
        )
        assert result.status != "unknown"
        assert result.amount_usd is not None
