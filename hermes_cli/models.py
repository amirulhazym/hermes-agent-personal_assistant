...
# D8 curated list for OpenCode Zen (locked 2026-07-25; see
# docs/superpowers/specs/2026-07-25-selected-model-contract-design.md
# decision D8 and D8a). This is a snapshot of OpenCode's current free
# tier; per D8a, if a model starts billing, that's a new user
...
# provider's live /v1/models discovery). They remain resolvable when typed
# explicitly (see DIRECT_ALIASES in model_switch.py) but must NOT surface in
# the picker unless the live catalog itself still serves them.
# Proven 2026-08-07: DeepSeek discontinued deepseek-chat / deepseek-reasoner
# from discovery on 2026-07-24 (they still work server-side, mapped to
...
def _filter_deprecated_aliases(
    provider: str, ids: list[str], live_ids: list[str] | None = None
) -> list[str]:
    """Drop deprecated aliases for *provider* from *ids*.

    An alias is kept only when the live catalog explicitly still serves it
    (``live_ids`` contains it). When ``live_ids`` is None (no live data),
    deprecated aliases are always dropped — discovery no longer lists them,
    so the picker must not offer them.
    """
    deprecated = _DEPRECATED_MODEL_ALIASES.get(provider)
    if not deprecated:
        return ids
    live = {m.lower() for m in (live_ids or [])}
    return [m for m in ids if m.lower() not in deprecated or m.lower() in live]


def curated_models(provider: str) -> list[str]:
    """Return the curated model list for a provider.

    This is the public single accessor for "which model IDs is the user
    permitted to select under this provider?" (S5 fix). It is the source
    of authority; the live ``/v1/models`` response only informs
    availability messaging (e.g. rate-limit warnings), not selectability.
    """
...
class SelectedRouteValidationError(Exception):
    """Raised when a provider+model combo fails curated-list validation.

    This is a fail-closed gate: if the route is not in a known curated
    list, the switch is rejected before any config write occurs.
    """
...
def validate_selected_route(provider: str, model: str) -> None:
    ...
    Fail-closed: if the provider has a curated list, the model MUST be in
    it. If no curated list exists, the model is allowed (will be validated
    later by the live API probe in validate_requested_model).

    Custom providers (custom:*) are always allowed — they are user-defined
    and validated at call time.

    Args:
        provider: The provider slug (e.g. "opencode-zen", "custom:ftf").
        model: The model ID to validate.

    Raises:
        SelectedRouteValidationError: If the model is not in the curated
            list for the given provider.
    """
    # Custom providers are always allowed
    if provider.startswith("custom:"):
        return
    # No curated list = no restriction (validated later by live probe)
...
_PROVIDER_MODELS: dict[str, list[str]] = {
    ...
    "opencode-zen": list(D8_OPENCODE_ZEN_FREE_MODELS),
    "opencode-go": [
...
CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ...
    ProviderEntry("deepseek",       "DeepSeek",                 "DeepSeek (V4 Flash/Pro, direct API)"),
...
def detect_provider_for_model(
    ...
    Returns ``(provider_id, model_name)`` — the model name may be remapped
    (e.g. bare model names → provider-prefixed aggregator slugs).
    Returns ``None`` when no confident match is found.
    ...
    # Deprecated legacy aliases must never reroute anywhere (e.g. OpenRouter).
    # _DEPRECATED_MODEL_ALIASES is the single registry of such ids; typed
    # input fails loud ("no model detected") instead of leaking to another
    # provider's stale catalog. (Full purge 2026-08-07.)
    if any(
        name.lower() in dep
        for dep in _DEPRECATED_MODEL_ALIASES.values()
    ):
        return None
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    if normalized == "opencode-go":
        ...
        return list(_PROVIDER_MODELS.get("opencode-go", []))
            ...
            try:
                ...
                if live:
                    if is_default_openai:
                        # ``/v1/models`` is a heterogeneous product catalog:
                        # embeddings, image, audio, moderation and other
                        # non-agent products are valid API models but cannot be
                        # selected by Hermes's tool-calling agent. Use the live
                        # endpoint as the availability source, then intersect it
                        # with Hermes's dynamic models.dev capability catalog.
                        # This avoids a hand-maintained OpenAI model allowlist
                        # while keeping the picker clean.
                        try:
                            from agent.models_dev import list_agentic_models

                            agentic = list_agentic_models("openai")
                        except Exception:
                            agentic = []
                        if agentic:
                            ...
                            filtered = [
                                m
                                for m in live
                                if m.lower() in agentic_lower
                                and "realtime" not in m.lower()
                            ]
                            if filtered:
                                return filtered
                        # Offline/stale models.dev fallback: retain the existing
                        # curated safety list rather than exposing raw products.
                        live_lower = {m.lower() for m in live}
                        curated = list(_PROVIDER_MODELS.get(normalized, []))
                        filtered = [m for m in curated if m.lower() in live_lower]
                        if filtered:
                            return filtered
                        # Account serves none of the known catalog entries (rare —
                        # e.g. a new organization-specific model rollout).
                    ...
                    if curated:
                        ...
                        for m in secondary:
                            if m.lower() not in merged_lower:
                                merged.append(m)
                                merged_lower.add(m.lower())
                        return _filter_deprecated_aliases(normalized, merged, live_ids=live)
                    return live
            # Use profile's fallback_models if defined
            if _p.fallback_models:
                return _filter_deprecated_aliases(
                    normalized, list(_p.fallback_models)
                )
    except Exception:
        pass
    ...
    if normalized in _MODELS_DEV_PREFERRED:
        return _filter_deprecated_aliases(
            normalized, _merge_with_models_dev(normalized, curated_static)
        )
    return _filter_deprecated_aliases(normalized, curated_static)
...
def cached_provider_model_ids(
    ...
    *,
    ...
    if (
        ...
        # Filter stale cache entries through the deprecated-alias gate so a
        # pre-fix cache can never serve deprecated aliases after an upgrade
        # without waiting for TTL expiry.
        return _filter_deprecated_aliases(normalized, list(entry["models"]))
