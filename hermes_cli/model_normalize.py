...
- **DeepSeek** accepts the first-class V-series IDs (``deepseek-v4-pro``,
  ``deepseek-v4-flash``, and any future ``deepseek-v<N>-*``). The legacy
  ``deepseek-chat`` / ``deepseek-reasoner`` aliases were discontinued by
  DeepSeek on 2026-07-24 and are fully removed (any other input falls back
...
# NOTE (2026-08-07): legacy ``deepseek-chat`` / ``deepseek-reasoner`` are
# fully removed — they were discontinued by DeepSeek on 2026-07-24 and no
# longer accepted anywhere in Hermes (picker, routing, normalization).
...
def _normalize_for_deepseek(model_name: str) -> str:
    ...
    - Everything else -> ``deepseek-v4-flash`` (the default/only chat model).
