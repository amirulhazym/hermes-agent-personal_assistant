...
class GatewaySlashCommandsMixin:
    ...
    async def _handle_status_command(self, event: MessageEvent) -> str:
        ...
        # Canonical (serving) model — P2/F6/F7. The provider may serve a
        # different model than the requested id. Only trust the cached
        # agent's response_model when there is no session override (an
        # override means the user just switched route; the cached agent's
        # canonical belongs to the pre-switch route). DB canonical_model is
        # a cold-agent fallback and is reset on model switch
        # (hermes_state.update_session_model).
        canonical_model = ""
        if (
            not override_model
            and status_agent is not None
            and status_agent is not _AGENT_PENDING_SENTINEL
        ):
            canonical_model = _clean_str(getattr(status_agent, "response_model", ""))
        if not canonical_model:
            canonical_model = _clean_str(session_row.get("canonical_model"))
        if canonical_model and canonical_model.lower() == model_name.lower():
            canonical_model = ""  # identical — plain display
        ...
        if model_name:
            if provider_name:
                if canonical_model:
                    model_line = t(
                        "gateway.status.model_provider_canonical",
                        model=model_name, canonical=canonical_model, provider=provider_name,
                    )
                else:
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        from gateway.observability import (
            current_context as _obs_current_context,
            hash_identifier as _obs_hash_identifier,
            new_correlation_id as _obs_new_correlation_id,
            record_event as _obs_record_event,
        )

        _obs_ctx = _obs_current_context()
        _model_correlation_id = _obs_ctx.get("correlation_id") or _obs_new_correlation_id()
        _model_started = time.monotonic()
        _model_platform = getattr(getattr(event.source, "platform", None), "value", None)
        _model_chat_hash = _obs_hash_identifier(event.source.chat_id)

        def _model_event(event_name: str, **fields: Any) -> None:
            fields.setdefault(
                "duration_ms",
                round((time.monotonic() - _model_started) * 1000, 3),
            )
            _obs_record_event(
                event_name,
                correlation_id=_model_correlation_id,
                platform=_model_platform,
                chat_id_hash=_model_chat_hash,
                **fields,
            )
        ...
        _model_event(
            "model_command_received",
            status="accepted",
            requested_model=model_input or None,
            requested_provider=explicit_provider or None,
            persist_scope="global" if persist_global else "session",
            ...
            if has_picker:
                ...
                if providers:
                    ...
                    async def _on_model_selected(
                        ...
                        if not result.success:
                            _model_event(
                                ...
                                duration_ms=round((time.monotonic() - _pick_started) * 1000, 3),
                            )
                    ...
                    _model_event(
                        ...
                        error_type="ModelPickerSendError",
                    )
        ...
        # Perform the switch
        _switch_started = time.monotonic()
        _model_event(
            "model_switch_started",
            source="typed",
        ...
        if not result.success:
            _model_event(
                ...
                requested_provider=explicit_provider or None,
                error_type="ModelSwitchError",
                duration_ms=round((time.monotonic() - _switch_started) * 1000, 3),
            )
            return t("gateway.model.error_prefix", error=result.error_message)
